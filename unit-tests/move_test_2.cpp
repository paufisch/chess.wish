...
//Pawn from (3,0) to (4,1)
TEST_F(Move_Test_2, Pawn_Move_1){

    Piece* moving_piece = _game_state->get_board()->get_piece(3,0);
    EXPECT_EQ(_game_state->move_piece(3,0,4,1), true);
    ...
    EXPECT_EQ(_game_state->get_board()->get_piece(3,0) == nullptr, true);
    EXPECT_EQ(_game_state->get_board()->get_piece(4,1) == moving_piece, true);

}

//Rook from (0,2) to (5,2)
TEST_F(Move_Test_2, Rook_Move_1){

    Piece* moving_piece = _game_state->get_board()->get_piece(0,2);
    EXPECT_EQ(_game_state->move_piece(0,2,5,2), true);
    EXPECT_EQ(_game_state->is_finished(), false);
    EXPECT_EQ(_game_state->get_loser() == nullptr, true);
    EXPECT_EQ(_game_state->get_round_number(), 1);
    EXPECT_EQ(_game_state->get_board()->get_piece(0,2) == nullptr, true);
    EXPECT_EQ(_game_state->get_board()->get_piece(5,2) == moving_piece, true);

}

//Rook from (0,2) to (0,3)
TEST_F(Move_Test_2, Rook_Move_2){

    Piece* moving_piece = _game_state->get_board()->get_piece(0,2);
    EXPECT_EQ(_game_state->move_piece(0,2,0,3), true);
    EXPECT_EQ(_game_state->is_finished(), false);
    EXPECT_EQ(_game_state->get_loser() == nullptr, true);
    EXPECT_EQ(_game_state->get_round_number(), 1);
    EXPECT_EQ(_game_state->get_board()->get_piece(0,2) == nullptr, true);
    EXPECT_EQ(_game_state->get_board()->get_piece(0,3) == moving_piece, true);
